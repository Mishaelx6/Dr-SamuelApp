    ...
    return (<div className="min-h-screen bg-gray-50">
      ...
      <section id="books" className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {booksLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (<div key={i} className="bg-white rounded-xl shadow-lg animate-pulse">
              ...
              {books.filter(book => book.featured).map((book) => (<BookCard key={book.id} book={book} onPreview={() => openFlipbook(book)} onPayment={() => setIsPaymentModalOpen(true)}/>))}
            </div>)}
        </div>
      </section>
      ...
      <section id="videos" className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {videosLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[1, 2, 3].map((i) => (<div key={i} className="bg-white rounded-xl shadow-lg animate-pulse">
              ...
              {videos.filter(video => video.featured).map((video) => (<VideoCard key={video.id} video={video}/>))}
            </div>)}
        </div>
      </section>
      ...
      {flipbookBook && (<FlipBookViewer book={flipbookBook} onClose={closeFlipbook} onPayment={() => {
                closeFlipbook();
                setIsPaymentModalOpen(true);
            }}/>)}
