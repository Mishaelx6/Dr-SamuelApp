      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (<div key={i} className="bg-white rounded-xl shadow-lg animate-pulse">
            ...
            </div>) : books.length === 0 ? (<div className="text-center py-16">
              ...
              {featuredBooks.length > 0 && (<div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {featuredBooks.map((book) => (<BookCard key={book.id} book={book} onPreview={() => openFlipbook(book)} onPayment={() => setIsPaymentModalOpen(true)}/>))}
                  </div>
                </div>)}

              {/* All Books */}
              {regularBooks.length > 0 && (<div>
                  <div className="flex items-center gap-3 mb-8">
                    <h2 className="text-3xl font-bold text-gray-900">All Books</h2>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {regularBooks.map((book) => (<BookCard key={book.id} book={book} onPreview={() => openFlipbook(book)} onPayment={() => setIsPaymentModalOpen(true)}/>))}
                  </div>
                </div>)}
            </div>)}
        </div>
